...
| `OURA_AGENT_ENGINE` | `false` | Движок ответов внешних агентов — см. «Движок агентов». Требует `OURA_SOURCES_FROM_UI=true` и ключа модели, иначе мост не стартует |
| `OURA_AGENT_API_KEY` | (обязателен при `OURA_AGENT_ENGINE=true`) | Ключ Anthropic API. Читается также из `ANTHROPIC_API_KEY`. Ключ платформенный: в события, логи и тексты ошибок не попадает |
| `OURA_AGENT_MODEL` | `claude-sonnet-5` | Модель для ответов агентов |
| `OURA_AGENT_AUTO_REPLY` | `false` | Отвечать клиенту без одобрения человеком. По умолчанию агент готовит черновик, а отправляет его оператор |
...
(`profile` — поля формы: `goal`, `role`, `tone`, `audience`, `instructions`,
`knowledge`; их читает движок ответов).
...
## Движок агентов (`OURA_AGENT_ENGINE=true`)

Внешний агент отвечает клиентам сам. Движок работает только поверх источников
из UI и только в комнатах лидов своего бота — командные каналы ему недоступны:
он видит лишь записи с префиксом `tg:<agentId>:`.

**Цикл по каждому лиду** (каждый `OURA_POLL_MS`):

1. Исполняются команды из комнаты (`/agent on`, `/agent off`) — только от операторов.
2. Досылается всё, что уже одобрено, но не ушло из-за сбоя связи.
3. Неотвеченные сообщения клиента склеиваются в одно обращение, к нему
   добавляется история диалога (последние 20 реплик) и профиль агента —
   и уходит один вызов модели.
4. Ответ публикуется в комнате под ключом агента.

**Черновики (по умолчанию).** Ответ появляется в комнате с подсказкой
«поставьте 👍, чтобы отправить клиенту» и клиенту НЕ уходит. Отправляет его
человек — реакцией `👍` (или `+`) на это сообщение. Реакции самого агента и
клиента не считаются. Доставленный черновик снимается: повторная реакция его
не отправит. Неодобренный черновик живёт сутки, а новый ответ отменяет
предыдущий — одобрить ответ на уже неактуальный вопрос нельзя.
`OURA_AGENT_AUTO_REPLY=true` убирает шаг одобрения: ответ уходит клиенту сразу.

**Передача человеку.** Клиент попросил живого сотрудника (`[NEED_HUMAN]`) или
разговор дошёл до конца сценария — агент замолкает, а команда получает в
комнате уведомление с итогом. Возврат — `/agent on` в комнате.

**Сведения о компании.** Что за бизнес, где находится, когда работает — это
общее для всех агентов и заполняется один раз в настройках сообщества.
Хранится событием NIP-78 (kind 30078, d-тег `oura:company`, открытый JSON:
`name`, `description`, `address`, `workingHours`, `contacts`) — новый kind ради
этого не нужен. Мост читает самую свежую редакцию тем же циклом, что и
описания агентов, и подставляет её в промпт каждого агента. Недоступность
описания агента не останавливает: он просто отвечает без общих сведений.

**Защиты**: на одно сообщение клиента ровно один ответ (сообщения берутся в
работу до вызова модели и освобождаются при её сбое); не более 5 вызовов
модели в минуту на лида; выключенный в интерфейсе агент (`isActive=false`) не
отвечает; расход токенов пишется в state по агенту.

Клиенту не показанные черновики в историю не попадают — агент не считает
сказанным то, чего клиент не видел.
