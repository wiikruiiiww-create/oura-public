...
import type { InboundMessage, InboundSource, OutboundMessage, OutboundSink } from "../types.js";
  ...
  private async route(
    ...
    try {
      if (req.method === "POST" && req.url === "/simulate") {
        ...
        try {
          body = JSON.parse(Buffer.concat(chunks).toString("utf8")) as Partial<InboundMessage>;
        } catch {
          res.writeHead(400).end(JSON.stringify({ error: "невалидный JSON" }));
          return;
        }
        if (typeof body.chatId !== "string" || typeof body.name !== "string" || typeof body.text !== "string") {
          res.writeHead(400).end(JSON.stringify({ error: "нужны chatId, name, text" }));
          return;
        }
        await onMessage({ chatId: body.chatId, name: body.name, text: body.text });
        res.writeHead(202).end(JSON.stringify({ ok: true }));
        return;
      }
      if (req.method === "GET" && req.url === "/outbox") {
        res.writeHead(200, { "content-type": "application/json" }).end(JSON.stringify(this.outbox));
        return;
      }
